...
def dataclass_to_struct_type(dataclass_type: Type) -> StructTypeProtocol:
    ...
    for field in dataclass_fields(dataclass_type):
        ...
        try:
            ...
            raise SchemaInferenceError(f"Cannot infer type for field {field.name}: {e}")
...
def pydantic_to_struct_type(model_type: Type) -> StructTypeProtocol:
    ...
    for field_name, field_info in model_type.model_fields.items():
        ...
        try:
            ...
            raise SchemaInferenceError(f"Cannot infer type for field {field_name}: {e}")
...
def infer_schema(
    ...
    if is_dataclass(model):
        ...
        try:
            ...
            raise SchemaInferenceError(f"Cannot infer schema from {model}: {e}")
    else:
